    ...
    switch(request.get_type())
    {
        ...
        case NodeRequestType::ICMP_NDP:
        {
            /* Only the first level of nodes can have MAC addresses, as all
               ...
               should be simulated. */
            auto found_node = std::find_if(this->_nodes.begin(), this->_nodes.end(), [&](const std::shared_ptr<NodeInfo>& node) {
            ...
            if (found_node != this->_nodes.end())
            {
                ...
                NodeReply reply(
                    ...
                    request.get_source_mac(), request.get_source_address(),
                    (*found_node)->get_mac_address(), request.get_destination_address()
                );
                return reply;
            }
            return NodeReply(NodeReplyType::NOREPLY);
